...
def compute_boundary_mfe_reflection(
    ...
    for i in range(4):
        ...
        print(line_projection_vector)

        residual = 0.5 * np.dot(
            line_projection_vector.T, line_projection_vector
        )  # Optimization target function

        print("residual: ", residual)
        print(_compute_line_point_distance(x_tmp, mfe_x, state_after_breach))

        residual_jacobian = _compute_line_point_projection_jacobian(
            x_tmp, mfe_x, mfe_x_jacobian, state_after_breach
        )

        gradient = residual_jacobian.T @ line_projection_vector

        print("Gradient: \n", gradient)

        # Project jacobian onto hyperplane generated by simplex face
        projected_residual_jacobian = _project_to_simplex_face(
            gradient, boundary_face_index
        )

        step_size = 0.1
        final_step_size = step_size
        current_residual_minimum = residual
        for j in range(10):
            tmp = x_tmp - j * step_size * projected_residual_jacobian
            new_line_vector = _compute_projection_onto_line_vector(
                tmp, _compute_mean_field_equation(tmp, r, r_tilde), state_after_breach
            )
            new_residual = 0.5 * new_line_vector.T @ new_line_vector

            if new_residual < current_residual_minimum:
                current_residual_minimum = new_residual
                final_step_size = j * step_size

        print("step_direction: \n", projected_residual_jacobian)
        print("final_step_size: \n", final_step_size)
        x_tmp -= final_step_size * projected_residual_jacobian

        print(f"x in iteration {i}: \n", x_tmp)
        print("sum over x: ", np.sum(x_tmp))
        print()
    x = 213
    ...
    for i in range(n_states):
        for j in range(n_states):
            res_jacobi[i, j] -= (
                ...
                * (
                    line_vector[j]
                    + vector_field_jacobian[:, j] @ (line_base_point - point)
                )
                + (line_base_point - point) @ line_vector * vector_field_jacobian[i, j]
            )
...
def _project_to_simplex_face(point: NDArray, face_index: int) -> NDArray:

    n_states = point.shape[0]
    projection = point.copy()

    projection[face_index] = 0

    correction = np.sum(projection) / (n_states - 1)
    for i in range(n_states):
        if i != face_index:
            projection[i] -= correction
